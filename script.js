...
function hydrateStoredTrack(record) {
  ...
  const track = {
    id: record.id,
    title: normalizeText(record.title) || '未命名歌曲',
    ...
    format: normalizeText(record.format) || 'AUDIO',
  ...
  if (record.sourceType === 'file') {
    ...
    if (!track.format || track.format === 'AUDIO') {
...
async function importFiles(fileList) {
  ...
  const statusParts = [`已加入 ${accepted.length} 首音乐`];

  if (failedCount > 0) {
    statusParts.push(`其中 ${failedCount} 首未写入本地存储`);
  } else {
    statusParts.push('并保存在本地浏览器');
  }

  if (rejected.length > 0) {
    statusParts.push(`忽略 ${rejected.length} 个非音频文件`);
  }
...
async function restorePersistedLibrary() {
  ...
  if (tracks.length) {
    setStatus(`已从本地浏览器恢复 ${tracks.length} 首音乐。`, 'success');
  } else if (storageAvailable) {
    setStatus('准备就绪，等待导入音乐；导入后会保存在本地浏览器。', 'info');
  } else {
    setStatus('本地存储不可用，导入的音频只能临时保留。', 'error');
  }
}
