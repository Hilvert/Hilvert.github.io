...
const els = {
  ...
  sleepStartButton: $('#sleep-start-button'),
  sleepClearButton: $('#sleep-clear-button'),
...
const STORAGE = {
  ...
  sleepTimerKey: 'audio-track-player.sleep-timer'
};
...
function clearSleepTimerTimers() {
  ...
  if (sleepTimerTickId) {
    window.clearInterval(sleepTimerTickId);
    sleepTimerTickId = null;
  }
}

function getSleepTimerRemainingMs() {
  return Math.max(0, state.sleepTimerEndAt - Date.now());
}

function saveSleepTimer() {
  try {
    if (state.sleepTimerEndAt > Date.now()) {
      window.localStorage.setItem(STORAGE.sleepTimerKey, JSON.stringify({
        endAt: state.sleepTimerEndAt,
        durationMinutes: state.sleepTimerDurationMinutes
      }));
    } else {
      window.localStorage.removeItem(STORAGE.sleepTimerKey);
    }
  } catch {
    return;
  }
...
function updateSleepTimerUI() {
  ...
  const isActive = state.sleepTimerEndAt > Date.now() && remainingMs > 0;

  els.sleepRemaining.textContent = isActive
    ? `剩余 ${formatDuration(remainingMs / 1000)}`
    : '未设置';
  els.sleepStartButton.textContent = isActive ? '重置' : '开始';
  els.sleepStartButton.disabled = !state.tracks.length;
  els.sleepClearButton.disabled = !isActive;
  els.sleepClearButton.setAttribute('aria-disabled', String(!isActive));
  els.sleepDuration.value = String(state.sleepTimerDurationMinutes || 30);
}
...
function scheduleSleepTimerTick() {
  ...
  sleepTimerTimeoutId = window.setTimeout(handleSleepTimerExpired, remainingMs);
  sleepTimerTickId = window.setInterval(() => {
    if (getSleepTimerRemainingMs() <= 0) {
      handleSleepTimerExpired();
      return;
    }
...
function restoreSleepTimer() {
  try {
    const raw = window.localStorage.getItem(STORAGE.sleepTimerKey);
    if (!raw) {
      updateSleepTimerUI();
      return;
    }
    ...
    const endAt = Number(stored?.endAt);
    ...
    state.sleepTimerEndAt = endAt;
    if (Number.isFinite(durationMinutes) && durationMinutes > 0) {
      state.sleepTimerDurationMinutes = durationMinutes;
    }
...
function startSleepTimer(minutes) {
  ...
  if (!Number.isFinite(parsedMinutes) || parsedMinutes <= 0) {
    setStatus('请选择有效的定时时长。', 'error');
    return;
  }

  if (!state.tracks.length) {
    setStatus('先导入一首音频或视频，再设置定时结束。', 'error');
    return;
  }

  state.sleepTimerDurationMinutes = parsedMinutes;
  state.sleepTimerEndAt = Date.now() + parsedMinutes * 60 * 1000;
  saveSleepTimer();
  scheduleSleepTimerTick();
  renderPlayer();
  setStatus(`定时结束已开启：${parsedMinutes} 分钟后自动停止。`, 'success');
}

function clearSleepTimer({ announce = true } = {}) {
  clearSleepTimerTimers();
  state.sleepTimerEndAt = 0;
  saveSleepTimer();
  renderPlayer();
  ...
  if (announce) {
    setStatus('已取消定时结束。', 'info');
  }
}

function handleSleepTimerStart() {
  startSleepTimer(els.sleepDuration.value);
}
...
function syncMediaElementForTrack(track) {
  ...
  if (isFullscreenActive() && track?.mediaType !== 'video') {
    document.exitFullscreen().catch(() => {});
  }
...
function isFullscreenActive() {
  return document.fullscreenElement === els.playerCard || document.fullscreenElement === els.videoPlayer;
}

function updateFullscreenUI() {
  const track = getCurrentTrack();
  const isVideo = track?.mediaType === 'video';
  const active = isVideo && isFullscreenActive();

  els.fullscreenButton.hidden = !isVideo;
  els.fullscreenButton.textContent = active ? '退出全屏' : '放大播放';
...
function toggleVideoFullscreen() {
  ...
  const target = typeof els.playerCard.requestFullscreen === 'function' ? els.playerCard : els.videoPlayer;
  if (typeof target.requestFullscreen === 'function') {
    target.requestFullscreen().catch(() => {
      setStatus('浏览器未允许视频放大播放。', 'error');
    });
    return;
  }

  if (typeof els.videoPlayer.webkitEnterFullscreen === 'function') {
    els.videoPlayer.webkitEnterFullscreen();
    return;
  }

  setStatus('当前浏览器不支持视频放大播放。', 'error');
}
...
async function removeTrack(trackId) {
  ...
  const media = getActiveMediaElement(track);
  const keepFullscreen = shouldContinue && nextCandidate?.mediaType === 'video';

  if (isFullscreenActive() && !keepFullscreen) {
    document.exitFullscreen().catch(() => {});
  }
...
function bindEvents() {
  ...
  els.fullscreenButton.addEventListener('click', toggleVideoFullscreen);
  els.sleepDuration.addEventListener('change', () => {
    state.sleepTimerDurationMinutes = Number(els.sleepDuration.value) || 30;
    updateSleepTimerUI();
  });
  els.sleepStartButton.addEventListener('click', handleSleepTimerStart);
  els.sleepClearButton.addEventListener('click', handleSleepTimerClear);
  ...
  els.videoPlayer.addEventListener('dblclick', toggleVideoFullscreen);
